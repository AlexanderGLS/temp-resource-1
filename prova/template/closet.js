var AvatarEditor = AvatarEditor || function ()
	{
		this.initialize();
		return this;
	};
...
AvatarEditor.Config = {
	...
	elements             : {
		...
		'list-color2'           : '#avatareditor-list-color2',
		'config'                : '#avatareditor-config'
	},
	json                 : {
		'figuredata'      : '/prova/json/figuredata.json',
		'itemset'         : '/prova/json/itemset.json',
		'avatarimage'     : 'https://images.bobbait.it/avatar/avatarimagejson.php',
		'avatarimage-part': 'https://images.bobbait.it/avatar/avatarimage_partson.php'
	}
};
...
AvatarEditor.prototype = {
	...
	initialize: function ()
	{
		...
		this.partTypeTabs = {
			primary  : $(AvatarEditor.Config.elements['settype-tabs-primary'] + ' > ul > li'),
			secondary: $(AvatarEditor.Config.elements['settype-tabs-secondary'] + ' > ul')
		};
		...
		$.getJSON(AvatarEditor.Config.json.figuredata, function (data)
		{
			...
			{
				if (i == 0) {
					avatareditor.active.partParent = $(this).children('a').attr('data-settype-parent');
		...
		{
			var crrSelector = $(AvatarEditor.Config.elements.config + ' select[name="config-handitem"]');
			$.each(data.crr, function (key, id)
			{
				crrSelector.append('<option value="' + id + '" data-style="background:url('+AvatarEditor.Config.json['avatarimage-part']+'?type=ri&id='+id+'&direction=5&action=crr&dyn=true) no-repeat 0px -52px">' + id + '</option>');
			});
			crrSelector.partIconSelectMenu({width: 'auto'});

			var sigSelector = $(AvatarEditor.Config.elements.config + ' select[name="config-signitem"]');
			$.each(data.sig, function (key, id)
			{
				sigSelector.append('<option value="' + id + '" data-style="background:url('+AvatarEditor.Config.json['avatarimage-part']+'?type=li&id='+id+'&direction=7&action=sig&dyn=true) no-repeat 0 0;">' + id + '</option>');
			});
			sigSelector.partIconSelectMenu({width: 'auto'});
		});

		$(document).on('click', AvatarEditor.Config.elements['list-parts'] + ' > ul > li > a',
			function (e)
			{
				e.preventDefault();
				avatareditor.changePart($(this).text());
				
				var maxcolore = $(this).parent().data("colormaxindex");
				if(maxcolore < 2){
					$(AvatarEditor.Config.elements['list-color']).removeClass("col-lg-2");
					$(AvatarEditor.Config.elements['list-color']).addClass("col-lg-4");
					$(AvatarEditor.Config.elements['list-color2']).hide();
				}else{
					$(AvatarEditor.Config.elements['list-color']).removeClass("col-lg-4");
					$(AvatarEditor.Config.elements['list-color']).addClass("col-lg-2");
					$(AvatarEditor.Config.elements['list-color2']).show();
				}
			});
		$(document).on('click', AvatarEditor.Config.elements['list-color'] + ' > ul > li > a',
			function (e)
			{
				e.preventDefault();
				avatareditor.changeColor($(this).text(), 0);
			});
		$(document).on('click', AvatarEditor.Config.elements['list-color2'] + ' > ul > li > a',
			function (e)
			{
				e.preventDefault();
				avatareditor.changeColor($(this).text(), 1);
			});
		this.partTypeTabs.primary.children('a').on('click', function (e)
		{
			...
			$(this).parent('li')
				...
				.addClass('active');
			avatareditor.partTypeTabs.secondary.removeClass('active')
				.filter('[data-parent-type=' + avatareditor.active.partParent + ']').addClass('active');
			avatareditor.changePartType();
		});
		this.partTypeTabs.secondary.find('li > a').on('click', function (e)
		{
			var gender = $(this).attr('data-gender');
			if (typeof gender != 'undefined' && avatareditor.active.gender != gender) {
		...
		$(AvatarEditor.Config.elements['preview-rect'] + ' > button')
			.on('click', function (e)
			{
				e.preventDefault();
				...
				if (avatareditor.active.options['action-primary'] == 'lay') {
					...
					if (dir == 'next') {
						avatareditor.active.direction.head =
							avatareditor.active.direction.head == 0 ? 7 : avatareditor.active.direction.head - 1;
						if ($(this).attr('name') == 'change-direction') {
							...
							avatareditor.active.direction.body =
								avatareditor.active.direction.body == 0 ? (isSit ? 6 : 7) : avatareditor.active.direction.body - 1 - (isSit ? 1 : 0);
						}
					}
					else {
						avatareditor.active.direction.head =
							avatareditor.active.direction.head == 7 ? 0 : avatareditor.active.direction.head + 1;
						if ($(this).attr('name') == 'change-direction') {
							if (isSit) {
								avatareditor.active.direction.head =
									avatareditor.active.direction.head >= 7 ? 0 : avatareditor.active.direction.head + 1;
							...
							avatareditor.active.direction.body =
								avatareditor.active.direction.body == (isSit ? 6 : 7) ? 0 : avatareditor.active.direction.body + 1 + (isSit ? 1 : 0);
						}
					}
				}
				avatareditor.updateRect();
				//if (avatareditor.active.direction.body != dir_body) avatareditor.buildPartsList(true);
			});

		$(AvatarEditor.Config.elements['container'] + ' button[name="figure-randomize"]')
			.on('click', function (e)
			{
				e.preventDefault();
				avatareditor.randomizeFigure();
	...
	{
		var object = $(AvatarEditor.Config.elements.form).find('input, textarea, select').filter('[name="' + name + '"]');
	...
	{
		var active = avatareditor.partTypeTabs.secondary.filter('[data-parent-type="' + this.active.partParent + '"]').find('li.active > a');
	...
	{
		var partList = $(AvatarEditor.Config.elements['list-parts'] + ' > ul > li').filter('.active').removeClass('active').end();
		...
		if (partID == 0 || typeof partID == 'undefined') {
			...
			this.active.figure[this.active.partType].color = [this.paletteIndex[this.active.paletteID]];
		} else {
	...
	changeColor: function (colorID, index)
	{
		if(index == 0){
			...
			$(AvatarEditor.Config.elements['list-color'] + ' > ul > li')
				.filter('.active-' + index).removeClass('active active-' + index).end()
				.filter('[data-id="' + colorID + '"]').addClass('active active-' + index);
		...
		{
			this.active.figure[this.active.partType].color[index] = Number(colorID);
			$(AvatarEditor.Config.elements['list-color2'] + ' > ul > li')
				.filter('.active-' + index).removeClass('active active-' + index).end()
				.filter('[data-id="' + colorID + '"]').addClass('active active-' + index);
			this.buildPartsList(true);
			this.updateRect();
	...
	randomizeFigure: function ()
	{
		...
		$.each(this.figuredata.settype, function (type, data)
		{
			...
			if (!data["mand_" + avatareditor.active.gender.toLowerCase() + "_1"]) {
				isSet = (getRandomArbitary(0, 100) <= AvatarEditor.Config.partSelectProbability);
			}
			if (isSet) {
				while (true) {
					part = list[getRandomArbitary(0, list.length - 1)];
					if (
						((!AvatarEditor.Config.club && data.set[part].club != 2) || AvatarEditor.Config.club) &&
						data.set[part].selectable == true &&
						(data.set[part].gender == avatareditor.active.gender || data.set[part].gender == 'U')
					)
						break;
				}
				var palette = Object.keys(avatareditor.figuredata.palette[data.paletteid]);
				while (true) {
					var color = palette[getRandomArbitary(0, palette.length - 1)];
					if (
						((!AvatarEditor.Config.club && avatareditor.figuredata.palette[data.paletteid][color].club != 2) || AvatarEditor.Config.club) &&
						avatareditor.figuredata.palette[data.paletteid][color].selectable == true) break;
				}
				...
				avatareditor.active.figure[type] = {
					...
					'id'   : 0,
					'color': [
						avatareditor.paletteIndex[data.paletteid],
						0,
						0
					]
				}
	...
	buildPartsList: function (isScrollHold)
	{
		...
		this.active.color = this.active.figure[this.active.partType].color;
		if (!this.figuredata.settype[this.active.partType]["mand_" + this.active.gender.toLowerCase() + "_1"]) {
		...
		$.each(this.figuredata.settype[this.active.partType].set, function (id, data)
		{
			if (AvatarEditor.Config.club != true && data.club == 2) return;
			if (data.selectable != true) return;
			if (data.gender != avatareditor.active.gender && data.gender != "U") return;
			var partsets = "";
			var maxindex = 0;
		...
		if(isScrollHold) var scrollTop = listPartsElement.children('ul').scrollTop();
		
		listPartsElement.children('ul').remove().end()
			.append(partLists)
			.find('.str-partname').text(this.active.partName);
	...
	buildPalette: function ()
	{
		...
		this.active.paletteID = this.figuredata.settype[this.active.partType].paletteid;
		this.active.color = typeof currentData != 'undefined' ? currentData.color : [this.paletteIndex[this.active.paletteID], 0, 0];
		...
		$(AvatarEditor.Config.elements['list-color']).children('ul').remove().end().append(partLists);
		...
		$(AvatarEditor.Config.elements['list-color2']).children('ul').remove().end().append(partLists2);
	},
	...
	updateRect: function ()
	{
		var input = {
			...
			'frame'         : (this.active.options.frame.length > 0) ? this.active.options.frame.join(',') : null,
			'headonly'      : Number(this.active.options.headonly),
			'img_format'    : this.active.options.format
		};
		$(AvatarEditor.Config.elements.form + ' input[name="figure"]').val(input.figure);
		$.ajax({
			dataType: 'json',
			...
			success : function (data)
			{
				...
				debugMessage += '</ul>';
				$(AvatarEditor.Config.elements['config'] + ' > fieldset[name="result"] > dl > dd')
					.filter('.result-process-time').html((data['process-time'] * 1000).toFixed(0) + 'ms').end()
					.filter('.result-error-message').html(data.error).end()
					.filter('.result-debug-message').html(debugMessage);
				$(AvatarEditor.Config.elements['preview-rect'])
					.removeClass('scale-l scale-n scale-s').children('img').remove().end()
				...
				$(AvatarEditor.Config.elements.container + ' .result-process-time').text((data['process-time'] * 1000).toFixed(0) + 'ms');
				$(AvatarEditor.Config.elements.container + ' .str-generator-version').text(data.version);
			}
		});
		var query = [];
		for (var i in input) {
			...
			if (i == 'head_direction' && this.active.direction.head == this.active.direction.body) continue;
			if (i != 'direction' && i != 'head_direction' && input[i] == 0) continue;
			query.push(i + '=' + input[i]);
		}
		$('input[name="avatareditor-image-url"]').val("http://" + location.host + "/avatarimage-demo?" + query.join('&'));
	},
	...
	updateConfig: function ()
	{
		...
		config.find('input[name="config-left-hand"]:checked').each(function (i)
		{
			...
			if (action == "sig") {
				action = action + "=" + avatareditor.getInputValue('config-signitem', '0');
				isCarry = true;
			}
			if(action != '') options['action-secondary'].push(action);
		});
		if (isSign) {
			config.find('.config-signitem').stop().fadeTo(100, 1).find('select, input').removeAttr('disabled');
		} else {
			config.find('.config-signitem').stop().fadeTo(100, 0.5).find('select, input').attr('disabled', 'disabled');
		}
		...
		config.find('input[name="config-right-hand"]:checked').each(function (i)
		{
			...
			if (action == "crr" || action == "drk") {
				action = action + "=" + avatareditor.getInputValue('config-handitem', '0');
				isCarry = true;
			}
			if(action != '') options['action-secondary'].push(action);
		});
		if (isCarry) {
			config.find('.config-handitem').stop().fadeTo(100, 1).find('select, input').removeAttr('disabled');
		} else {
			config.find('.config-handitem').stop().fadeTo(100, 0.5).find('select, input').attr('disabled', 'disabled');
		}
		...
		if (options['action-primary'] == 'wlk' || options['action-primary'] == 'swm') {
			var frame = this.getInputValue('config-frame-' + options['action-primary'], '0');
			if (frame > 0) options.frame.push(options['action-primary'] + "=" + frame);
		}
